...
public class MainFrame extends JFrame {
    ...
    public MainFrame(Double[] coefficients) {
        super("Tabulation of a polynomial on a segment according to the Gorners scheme");
        ...
        Action saveToTextAction = new AbstractAction("Save to the text file") {
        ...
        Action saveToGraphicsAction = new AbstractAction("Save data for graf") {
            public void actionPerformed(ActionEvent event) {
                if (fileChooser == null) {
                    fileChooser = new JFileChooser();
                    fileChooser.setCurrentDirectory(new File("."));
        ...
        Action searchValueAction = new AbstractAction("Find X in result") { // Создать новое действие по поиску значений многочлена
            public void actionPerformed(ActionEvent event) {
                String value = JOptionPane.showInputDialog(MainFrame.this, "Enter a polynomial","Searching...", JOptionPane.QUESTION_MESSAGE); // Запросить пользователя ввести искомую строку
                renderer.setNeedle(value); // Установить введенное значение в качестве иголки
                getContentPane().repaint(); // Обновить таблицу
            }
        };

        AbstractAction aboutProgrammAction = new AbstractAction("Creator") {
            public void actionPerformed(ActionEvent event) {
                String surname = "Волк";
                String group = "8";
                ...
                JOptionPane.showMessageDialog(null, panel, "Creator", JOptionPane.PLAIN_MESSAGE);
            }
        };
        ...
        JLabel labelForFrom = new JLabel("X changing on interval from:"); //Создать область с полями ввода для границ отрезка и шага Создать подпись для ввода левой границы отрезка
        ...
        JLabel labelForStep = new JLabel("step:"); //Создать подпись для ввода шага табулирования
        ...
        JButton buttonCalc = new JButton("Calculate"); //Создать кнопку "Вычислить"
        buttonCalc.addActionListener(new ActionListener() { //Задать действие на нажатие "Вычислить" и привязать к кнопке
            public void actionPerformed(ActionEvent ev) {
                try {
                    ...
                    JOptionPane.showMessageDialog(MainFrame.this, "Error in the format of writing a number","Error", JOptionPane.WARNING_MESSAGE);
                }
            }
        });

        JButton buttonReset = new JButton("Clear everything"); //Создать кнопку "Очистить поля"
